...
import { Play, Mic, CheckCircle, ChevronLeft, HelpCircle, FileAudio, AlignLeft, ListChecks, Type, FileText, Send, Loader2 } from 'lucide-react';
...
export default function UserLessonsTab() {
  ...
  const calculateAndSubmitScore = async () => {
    ...
    const tests = activeLesson.tests || [];
    const allQuestions = [...prepTasks, ...quizzes, ...tests];
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`https://hyggee.ir/api/v1/lessons/${activeLesson.id}/submit`, {
         method: 'POST',
         headers: {
           'Authorization': `Bearer ${token}`,
           'Content-Type': 'application/json'
       ...
       print("Failed to sync score to DB context:", err);
    } finally {
  ...
  const handlePostToCommunity = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://hyggee.ir/api/v1/ws/chat/general/broadcast`, {
        method: 'POST',
  ...
  const renderQuestionBlock = (title, icon, questions, answers, setAnswers) => {
    ...
    return (
      <div className="bg-[#2f3e40]/60 border border-white/10 rounded-3xl p-5 flex flex-col gap-4 mt-2">
        ...
        {questions.map((q, qIdx) => {
          ...
          return (
            <div key={qIdx} className="flex flex-col gap-3 pb-3 border-b border-white/5 last:border-0 last:pb-0">
              {type !== 'DRAGGING_WORDS' && <p className="font-medium text-sm text-white/90">{qIdx + 1}. {qText}</p>}
                ...
                <div className="flex flex-col gap-2">
                  {q.options.map((opt, oIdx) => {
                    ...
                    return (
                      <button
                        key={oIdx}
                        onClick={() => handleSelect(setAnswers, qIdx, oIdx)}
                        className={`text-left p-3 rounded-xl border text-sm transition-all focus:outline-none cursor-pointer ${
                          isSelected 
              ...
              {type === 'SHORT_ANSWER' && (
                <input 
                  ...
                  className="w-full py-2.5 px-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/40 text-sm focus:border-hygge-teal/50 focus:outline-none"
                />
              )}
              ...
              {type === 'DRAGGING_WORDS' && (
                <div className="flex flex-col gap-3">
                  ...
                  <DropBlankUI 
                     question={q} 
                     value={answers[qIdx] || {}} 
                     onChange={(v) => setAnswers(prev => ({...prev, [qIdx]: v}))} 
                  />
                </div>
              )}
  ...
  if (activeLesson) {
    ...
    return (
      <motion.div 
        className="flex flex-col h-full inset-0 z-50 fixed bg-gradient-to-b from-[#415354] via-[#546E6D] to-[#718E8C] overflow-y-auto"
      ...
      >
        ...
        <div className="p-4 md:p-6 pb-32 flex flex-col gap-4 max-w-2xl mx-auto w-full">
          {renderQuestionBlock("1. Preparation Task", ListChecks, prepTasks, prepAnswers, setPrepAnswers)}

          <div className="bg-[#2f3e40]/60 border border-white/10 rounded-3xl p-5 flex flex-col gap-4 mt-2">
            <div className="flex items-center gap-2 mb-1">
              <Play className="w-5 h-5 text-amber-400" />
              <h4 className="font-bold text-white text-base">2. Video Material</h4>
            </div>
            {fullVideoUrl ? (
          ...
          <div className="bg-[#2f3e40]/60 border border-white/10 rounded-3xl p-5 flex flex-col gap-2 mt-2">
             <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="font-bold text-white text-base">3. Description</h4>
            </div>
            <p className="text-sm text-white/70 font-light leading-relaxed">{activeLesson.description}</p>
          </div>

          {renderQuestionBlock("4. Lesson Quiz", HelpCircle, activeLesson.quizzes, quizAnswers, setQuizAnswers)}
          ...
          {renderQuestionBlock("6. Final Test", FileText, activeLesson.tests, testAnswers, setTestAnswers)}

          {score === null ? (
            <button
              onClick={calculateAndSubmitScore}
              disabled={isSubmitting}
              className="w-full py-4 bg-hygge-teal hover:bg-hygge-teal/90 text-white font-bold text-sm rounded-xl shadow-md transition mt-4 cursor-pointer disabled:opacity-50"
            >
              {isSubmitting ? "Syncing Answers..." : "Submit All Answers"}
            </button>
          ) : (
            <div className={`p-5 rounded-3xl border flex items-center justify-between font-bold text-lg mt-4 ${
          ...
          {/* 7. Media Teaching Segment */}
          <div className="bg-[#2f3e40]/60 border border-white/10 rounded-3xl p-5 flex flex-col gap-4 mt-4">
            <div className="flex items-center gap-2">
              <FileAudio className="w-5 h-5 text-amber-400" />
              <h4 className="font-bold text-white text-base">7. Media Teaching Segment</h4>
            </div>
            ...
            {postSuccess ? (
              ...
              <button onClick={handlePostToCommunity} disabled={!teachingSegment.trim() && !teachingMedia} className="self-end px-6 py-2.5 bg-hygge-teal hover:bg-hygge-teal/90 disabled:opacity-50 border border-white/10 text-white rounded-xl text-sm font-medium transition cursor-pointer flex items-center gap-2">Post to Feed</button>
            )}
          </div>
    ...
    <div className="flex flex-col gap-6" id="user-lessons-tab">
      ...
      {lessons.length === 0 ? (
        ...
        <div className="flex flex-col gap-4">
          {lessons.map((lesson) => (
            <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} whileTap={{ scale: 0.98 }} onClick={() => openLesson(lesson)} className="bg-[#2f3e40]/60 hover:bg-[#2f3e40] border border-white/10 rounded-3xl p-4 flex flex-col gap-3 transition cursor-pointer group shadow-sm flex-grow">
              <div className="flex gap-4 items-center">
                <div className="w-16 h-16 rounded-2xl bg-[#546E6D]/50 border border-white/5 flex items-center justify-center shrink-0 relative overflow-hidden group-hover:bg-[#546E6D] transition shadow-inner">
                  {lesson.completed && (
                ...
                <div className="flex flex-col flex-grow min-w-0">
                  ...
                  <h5 className="font-bold text-white text-base leading-snug group-hover:text-hygge-cream transition truncate">{lesson.title}</h5>
                  <p className="text-xs text-white/50 font-light mt-1 line-clamp-1">{lesson.description}</p>
                </div>
              </div>
              <div className="w-full h-1 bg-white/5 rounded-full mt-1 overflow-hidden">
                 <div className={`h-full rounded-full ${lesson.completed ? 'bg-hygge-teal w-full' : 'bg-hygge-teal/40 w-1/4'}`} />
              </div>
            </motion.div>
          ))}
        </div>
